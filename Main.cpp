...
int main()
{
    int stateValue = WindowState::setMenu;
    ...
    while (window.isOpen())
    {
        ...
        if(stateValue == WindowState::setMenu)
        {
            ...
            MenuStack.emplace(VIEW_WIDTH, VIEW_HEIGHT);
        ...
        if(stateValue == WindowState::setGame)
        {
            ...
            GameStack.emplace(&playerTexture, &enemyTexture, &groundTexture, &floorTexture, &bulletTexture, &healthTexture);
            stateValue = Game;
        }
        if(stateValue == WindowState::Game)
        {
            GameStack.top().Loop(window, view, deltaTime);

            if(GameStack.top().getStateValue() == setMenu)
            {
                GameStack.pop();
